...
public class SetupCommand extends ListenerAdapter {
	...
	public void onGuildMessageReceived(GuildMessageReceivedEvent e) {
		...
		if (args[0].equalsIgnoreCase(Croupier.prefix + "setup")) {
			...
			Document doc = collection.find(Filters.eq("guildid", e.getGuild().getIdLong())).first();
			...
			if (doc == null) {
				...
				e.getChannel().sendMessage(msg.build()).reference(e.getMessage()).mentionRepliedUser(false)
						.queue(message -> {
							...
							e.getGuild().createCategory("🎲Roulette Rounds🎲")
				...
				error.setTitle("There is already a Game created");
				error.setDescription("<#" + doc.getLong("channelid") + ">");
